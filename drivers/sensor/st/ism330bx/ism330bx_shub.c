...
static int ism330bx_shub_write_target_reg(const struct device *dev, uint8_t trgt_addr,
					  uint8_t trgt_reg, uint8_t *value, uint16_t len);
static int ism330bx_shub_read_target_reg(const struct device *dev, uint8_t trgt_addr,
					 uint8_t trgt_reg, uint8_t *value, uint16_t len);
static void ism330bx_shub_enable(const struct device *dev, uint8_t enable);

/* ST HAL skips this register, only supports it via the slower ism330bx_sh_status_get() */
static int32_t ism330bx_sh_status_mainpage_get(stmdev_ctx_t *ctx, ism330bx_status_master_t *val)
{
	return ism330bx_read_reg(ctx, ism330bx_STATUS_MASTER_MAINPAGE, (uint8_t *)val, 1);
}

/*
 * LIS2MDL magn device specific part
 */
#ifdef CONFIG_ism330bx_EXT_LIS2MDL
...
static int ism330bx_lis2mdl_init(const struct device *dev, uint8_t i2c_addr)
{
	...
	ism330bx_shub_write_target_reg(dev, i2c_addr, LIS2MDL_CFG_REG_A, mag_cfg, 1);
	...
	ism330bx_shub_write_target_reg(dev, i2c_addr, LIS2MDL_CFG_REG_A, mag_cfg, 2);
...
static int ism330bx_lis2mdl_odr_set(const struct device *dev, uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
	...
	cfg = (odr << 2);
	ism330bx_shub_write_target_reg(dev, i2c_addr, LIS2MDL_CFG_REG_A, &cfg, 1);

	ism330bx_shub_enable(dev, 1);
	return 0;
}

static int ism330bx_lis2mdl_conf(const struct device *dev, uint8_t i2c_addr,
				 enum sensor_channel chan, enum sensor_attribute attr,
				 const struct sensor_value *val)
{
	switch (attr) {
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		return ism330bx_lis2mdl_odr_set(dev, i2c_addr, val->val1);
	default:
...
#endif /* CONFIG_ism330bx_EXT_LIS2MDL */

/*
 * HTS221 humidity device specific part
 */
#ifdef CONFIG_ism330bx_EXT_HTS221
...
static int ism330bx_hts221_read_conv_data(const struct device *dev, uint8_t i2c_addr)
{
	struct ism330bx_data *data = dev->data;
	uint8_t buf[16], i;
	...
	for (i = 0; i < sizeof(buf); i += 7) {
		...
		if (ism330bx_shub_read_target_reg(
...
static int ism330bx_hts221_init(const struct device *dev, uint8_t i2c_addr)
{
	uint8_t hum_cfg;

	/* configure ODR and BDU */
	hum_cfg = HTS221_ODR_1HZ | HTS221_BDU | HTS221_PD;
	ism330bx_shub_write_target_reg(dev, i2c_addr, HTS221_REG_CTRL1, &hum_cfg, 1);

	return ism330bx_hts221_read_conv_data(dev, i2c_addr);
}

static const uint16_t hts221_map[] = {0, 1, 7, 12};

static int ism330bx_hts221_odr_set(const struct device *dev, uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
	...
	ism330bx_shub_write_target_reg(dev, i2c_addr, HTS221_REG_CTRL1, &cfg, 1);

	ism330bx_shub_enable(dev, 1);
	return 0;
}

static int ism330bx_hts221_conf(const struct device *dev, uint8_t i2c_addr,
				enum sensor_channel chan, enum sensor_attribute attr,
				const struct sensor_value *val)
{
	switch (attr) {
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		return ism330bx_hts221_odr_set(dev, i2c_addr, val->val1);
	default:
...
#endif /* CONFIG_ism330bx_EXT_HTS221 */

/*
 * LPS22HB baro/temp device specific part
 */
#ifdef CONFIG_ism330bx_EXT_LPS22HB
...
static int ism330bx_lps22hb_init(const struct device *dev, uint8_t i2c_addr)
{
	uint8_t baro_cfg[2];

	/* sw reset device */
	baro_cfg[0] = LPS22HB_SW_RESET;
	ism330bx_shub_write_target_reg(dev, i2c_addr, LPS22HB_CTRL_REG2, baro_cfg, 1);

	k_sleep(K_MSEC(1)); /* turn-on time in ms */

	/* configure device */
	baro_cfg[0] = LPS22HB_ODR_10HZ | LPS22HB_LPF_EN | LPS22HB_BDU_EN;
	ism330bx_shub_write_target_reg(dev, i2c_addr, LPS22HB_CTRL_REG1, baro_cfg, 1);

	return 0;
}
#endif /* CONFIG_ism330bx_EXT_LPS22HB */

/*
 * LPS22HH baro/temp device specific part
 */
#ifdef CONFIG_ism330bx_EXT_LPS22HH
...
static int ism330bx_lps22hh_init(const struct device *dev, uint8_t i2c_addr)
{
	uint8_t baro_cfg[2];

	/* sw reset device */
	baro_cfg[0] = LPS22HH_SW_RESET;
	ism330bx_shub_write_target_reg(dev, i2c_addr, LPS22HH_CTRL_REG2, baro_cfg, 1);
	...
	ism330bx_shub_write_target_reg(dev, i2c_addr, LPS22HH_CTRL_REG1, baro_cfg, 1);
...
static int ism330bx_lps22hh_odr_set(const struct device *dev, uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
	...
	ism330bx_shub_write_target_reg(dev, i2c_addr, LPS22HH_CTRL_REG1, &cfg, 1);

	ism330bx_shub_enable(dev, 1);
	return 0;
}

static int ism330bx_lps22hh_conf(const struct device *dev, uint8_t i2c_addr,
				 enum sensor_channel chan, enum sensor_attribute attr,
				 const struct sensor_value *val)
{
	switch (attr) {
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		return ism330bx_lps22hh_odr_set(dev, i2c_addr, val->val1);
	default:
...
#endif /* CONFIG_ism330bx_EXT_LPS22HH */

/*
 * LPS22DF baro/temp device specific part
 */
#ifdef CONFIG_ism330bx_EXT_LPS22DF
...
static int ism330bx_lps22df_init(const struct device *dev, uint8_t i2c_addr)
{
	uint8_t baro_cfg[2];

	/* sw reset device */
	baro_cfg[0] = LPS22DF_SW_RESET;
	ism330bx_shub_write_target_reg(dev, i2c_addr, LPS22DF_CTRL_REG2, baro_cfg, 1);
	...
	ism330bx_shub_write_target_reg(dev, i2c_addr, LPS22DF_CTRL_REG1, baro_cfg, 1);
...
static int ism330bx_lps22df_odr_set(const struct device *dev, uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
	...
	ism330bx_shub_write_target_reg(dev, i2c_addr, LPS22DF_CTRL_REG1, &cfg, 1);

	ism330bx_shub_enable(dev, 1);
	return 0;
}

static int ism330bx_lps22df_conf(const struct device *dev, uint8_t i2c_addr,
				 enum sensor_channel chan, enum sensor_attribute attr,
				 const struct sensor_value *val)
{
	switch (attr) {
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		return ism330bx_lps22df_odr_set(dev, i2c_addr, val->val1);
	default:
...
#endif /* CONFIG_ism330bx_EXT_LPS22DF */

/* List of supported external sensors */
static struct ism330bx_shub_slist {
...
#ifdef CONFIG_ism330bx_EXT_LIS2MDL
	{
		...
		.out_data_addr = 0x68,
		.out_data_len = 0x06,
		.dev_init = (ism330bx_lis2mdl_init),
		.dev_conf = (ism330bx_lis2mdl_conf),
	},
#endif /* CONFIG_ism330bx_EXT_LIS2MDL */
...
#ifdef CONFIG_ism330bx_EXT_HTS221
	{
		...
		.out_data_len = 0x02,
		.dev_init = (ism330bx_hts221_init),
		.dev_conf = (ism330bx_hts221_conf),
	},
#endif /* CONFIG_ism330bx_EXT_HTS221 */
...
#ifdef CONFIG_ism330bx_EXT_LPS22HB
	{
		...
		.out_data_addr = 0x28,
		.out_data_len = 0x05,
		.dev_init = (ism330bx_lps22hb_init),
	},
#endif /* CONFIG_ism330bx_EXT_LPS22HB */
...
#ifdef CONFIG_ism330bx_EXT_LPS22HH
	{
		...
		.out_data_addr = 0x28,
		.out_data_len = 0x05,
		.dev_init = (ism330bx_lps22hh_init),
		.dev_conf = (ism330bx_lps22hh_conf),
	},
#endif /* CONFIG_ism330bx_EXT_LPS22HH */
...
#ifdef CONFIG_ism330bx_EXT_LPS22DF
	{
		...
		.out_data_addr = 0x28,
		.out_data_len = 0x05,
		.dev_init = (ism330bx_lps22df_init),
		.dev_conf = (ism330bx_lps22df_conf),
	},
#endif /* CONFIG_ism330bx_EXT_LPS22DF */
};

static int ism330bx_shub_wait_completed(stmdev_ctx_t *ctx)
{
	ism330bx_status_master_t status;
	...
	do {
		...
		k_msleep(1);
		ism330bx_sh_status_mainpage_get(ctx, &status);
...
static void ism330bx_shub_enable(const struct device *dev, uint8_t enable)
{
	...
	if (ism330bx_sh_master_set(ctx, enable) < 0) {
		LOG_DBG("shub: failed to set master on");
		ism330bx_mem_bank_set(ctx, ism330bx_MAIN_MEM_BANK);
		return;
	}
...
static int ism330bx_shub_check_slv0_nack(stmdev_ctx_t *ctx)
{
	ism330bx_all_sources_t status;

	if (ism330bx_all_sources_get(ctx, &status) < 0) {
					 ...
					 uint8_t trgt_reg, uint8_t *value, uint16_t len)
{
	const struct ism330bx_config *cfg = dev->config;
	stmdev_ctx_t *ctx = (stmdev_ctx_t *)&cfg->ctx;
	ism330bx_sh_cfg_read_t trgt_cfg;

	trgt_cfg.slv_add = trgt_addr;
	trgt_cfg.slv_subadd = trgt_reg;
	trgt_cfg.slv_len = len;

	ism330bx_sh_slv_cfg_read(ctx, 0, &trgt_cfg);

	/* turn SH on, wait for shub i2c read to finish */
	ism330bx_shub_enable(dev, 1);
	ism330bx_shub_wait_completed(ctx);

	/* read data from external target */
	if (ism330bx_sh_read_data_raw_get(ctx, value, len) < 0) {
		LOG_DBG("shub: error reading sensor data");
		return -EIO;
	}

	if (ism330bx_shub_check_slv0_nack(ctx) < 0) {
		ism330bx_shub_enable(dev, 0);
		return -EIO;
	}
					  ...
					  uint8_t trgt_reg, uint8_t *value, uint16_t len)
{
	const struct ism330bx_config *cfg = dev->config;
	stmdev_ctx_t *ctx = (stmdev_ctx_t *)&cfg->ctx;
	ism330bx_sh_cfg_write_t trgt_cfg;
	uint8_t cnt = 0U;
	...
	while (cnt < len) {
		...
		ism330bx_sh_cfg_write(ctx, &trgt_cfg);
...
static int ism330bx_shub_set_data_channel(const struct device *dev)
{
	...
	ism330bx_sh_slave_connected_t aux = ism330bx_SLV_0_1_2;

	if (ism330bx_sh_slave_connected_set(ctx, aux) < 0) {
...
int ism330bx_shub_get_idx(const struct device *dev, enum sensor_channel type)
{
	uint8_t n;
	struct ism330bx_data *data = dev->data;
	struct ism330bx_shub_slist *sp;
...
int ism330bx_shub_fetch_external_devs(const struct device *dev)
{
	uint8_t n;
	const struct ism330bx_config *cfg = dev->config;
	stmdev_ctx_t *ctx = (stmdev_ctx_t *)&cfg->ctx;
	...
	/* read data from external target */
	if (ism330bx_mem_bank_set(ctx, ism330bx_SENSOR_HUB_MEM_BANK) < 0) {
	...
	for (n = 0; n < data->num_ext_dev; n++) {
		...
		if (ism330bx_read_reg(ctx, sp->sh_out_reg, data->ext_data[n], sp->out_data_len) <
		    0) {
			LOG_DBG("shub: failed to read sample");
			(void)ism330bx_mem_bank_set(ctx, ism330bx_MAIN_MEM_BANK);
			return -EIO;
		}
	}

	return ism330bx_mem_bank_set(ctx, ism330bx_MAIN_MEM_BANK);
}

int ism330bx_shub_config(const struct device *dev, enum sensor_channel chan,
			 enum sensor_attribute attr, const struct sensor_value *val)
{
	struct ism330bx_data *data = dev->data;
	struct ism330bx_shub_slist *sp = NULL;
	uint8_t n;
...
int ism330bx_shub_init(const struct device *dev)
{
	struct ism330bx_data *data = dev->data;
	const struct ism330bx_config *cfg = dev->config;
	stmdev_ctx_t *ctx = (stmdev_ctx_t *)&cfg->ctx;
	...
	/*
	 * This must be set or ism330bx_shub_write_target_reg() will
	 * repeatedly write the same regi
	 */
	if (ism330bx_sh_write_mode_set(ctx, ism330bx_ONLY_FIRST_CYCLE) < 0) {
		LOG_DBG("shub: error setting write once");
		return -EIO;
	}

	for (n = 0; n < ARRAY_SIZE(ism330bx_shub_slist); n++) {
		if (data->num_ext_dev >= ism330bx_SHUB_MAX_NUM_TARGETS) {
			break;
		}

		chip_id = 0;
		sp = &ism330bx_shub_slist[n];
		...
		for (i = 0U; i < ARRAY_SIZE(sp->i2c_addr); i++) {
			if (ism330bx_shub_read_target_reg(dev, sp->i2c_addr[i], sp->wai_addr,
							  &chip_id, 1) < 0) {
	...
	for (n = 0, regn = 0; n < data->num_ext_dev; n++) {
		sp = &ism330bx_shub_slist[data->shub_ext[n]];
		sp->sh_out_reg = ism330bx_SENSOR_HUB_1 + regn;
		regn += sp->out_data_len;
		sp->dev_init(dev, sp->ext_i2c_addr);
	}

	ism330bx_shub_set_data_channel(dev);
